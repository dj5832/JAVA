...
public class VariableInit {
	...
	VariableInit(){
		var = 50;
//		staticVar = 60;
		// 값을 공유해야하는 클래스 변수가 객체 생성 시 마다 계속 초기화되기 때문에 클래스 변수를 생성자에서 초기화 하는 것은 좋지 않다.
...
class Init {
	...
	Init(int a, int b, int c){
		this.a = a;	// 전역변수를 사용하기 위해서 this를 붙인다. 변수의 이름이 같을 대 구분하기 위해 사용한다.
		this.b = b;	// this는 클래스에 있는 객체의 주소를 저장하고 있는 하나의 변수
		this.c = c; // 앞에있는 c 는 클래스의 변수, 뒤에 있는 c는 파라미터 변수
		// this : 인스턴스 변수와 지역변수의 이름이 같을 대 둘을 구분하기 위해 사용한다.
