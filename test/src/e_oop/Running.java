...
public class Running {
	...
	boolean power() {
		...
		if(wtf.equals("on")) {
			...
			if(cal > 100) {
				System.out.println(cal + "Kcal를 소모하셨습니다. 앞으로도 꾸준히!!");
			}else {
				System.out.println(cal + "Kcal를 소모하셨습니다. 조금 더 열심히 해주세요!!");
			}
			this.power = false;
		}
	...
	public static void main(String[] args) {
		...
		String option = "[전원 키기 : on / 전원 끄기 off / 속도 조절 : spup OR spdown OR \"속도입력\" 입력 / 각도 조절 : incup OR incdown";
		...
		while(true) {
			...
			if(wtf.equals("on") || wtf.equals("off")) {
				...
				if(run.power()) { // run.power 가 true 이면 실행
					System.out.println("런닝머신을 시작합니다.");
				}else {
